...
public class LinguisticTreeModel implements TreeViewModel 
{
	...
	{
		List<LinguisticTreeNode> dataList = new ArrayList<LinguisticTreeNode>() ;
	 ...
	 */
	protected void fillDataList(List<LinguisticTreeNode> dataList, final List<LemmaWithInflections> aSynonyms)
	{
		if ((null == aSynonyms) || (null == dataList) || aSynonyms.isEmpty())
			return ;

		// Add all lemmas as children of the root node
		//
		for (LemmaWithInflections lemma : aSynonyms)
		{
			...
			if (false == lemma.getInflections().isEmpty())
			{
				for (Iterator<FlexWithTraits> itInflection = lemma.getInflections().iterator() ; itInflection.hasNext() ; )
				{
					...
					if (false == lemma.getTraits().isEmpty())
					{
						for (TripleWithLabel triple : flex.getTraits())
						{
							LinguisticTreeNode traitNode = new LinguisticTreeNode(triple, flexNode) ; 
							// dataList.add(traitNode) ;
						}
					}
				}
